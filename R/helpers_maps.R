...
make_map_transportation = function(){
  ...
  project_locations = file_projects_spatial %>%
    ...
    map = mapview(spatial_sidewalk, zcol = "NEED_IND", lwd = 3, homebutton = F, label = "Sidewalk", layer.name = "Sidewalk")  +
...
make_map_environ = function(){
  ...
  spatial_multirisk = spatial_multirisk %>%
    mutate(consequence_avg = dgt2(consequence_avg)
           ,across(ends_with("_risk"), dgt2)) %>%
  ...
  map = mapview(spatial_fishbarriers, homebutton = F, label = "Fish Barriers", layer.name = "Fish Barriers")  +
    ...
    mapview(spatial_multirisk, homebutton = F, zcol = "consequence_avg", lwd = 3, layer.name = "Climate Risk") +
